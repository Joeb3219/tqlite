...
export class DatabaseFile extends File {
    readDatabase() {
        ...
        const pages = _.range(0, header.databaseFileSizeInPages).map(pageIdx => {
            const page = this.readPage(header, pageIdx);
            const result = DatabaseFileBTreePageUtil.parseBTreePage(page, pageIdx, header);
            return result;
        })
    ...
    readPage(header: DatabaseHeader, pageNumber: number): Buffer {
        ...
        const modifiedStartIdx = pageNumber === 0 ? realStartIdx + 100 : realStartIdx;

        // We aren't always going to return `header.pageSizeBytes` bytes.
        // This is fine.
        return this.data.subarray(realStartIdx, realStartIdx + header.pageSizeBytes);
    }
